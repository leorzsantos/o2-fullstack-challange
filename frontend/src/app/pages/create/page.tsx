...
import { ProdutoRequestDto } from "@/app/models/produto";
import { SubmitErrorHandler, SubmitHandler, useForm, Controller } from "react-hook-form";
import { createProduct } from "@/app/services/produto.service";

export default function CreatePage() {
    const { control, register, handleSubmit, formState: { errors }, reset } = useForm<ProdutoRequestDto>({
        defaultValues: {
            nome: "",
            descricao: "",
            quantidade: 0,
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit(onSubmit, onError)} action="" className="flex gap-3 flex-col">
                ...
                <button type="submit" className="bg-black text-white mt-5 px-4 py-1 rounded-md cursor-pointer">Adicionar Produto</button>
            </form>
